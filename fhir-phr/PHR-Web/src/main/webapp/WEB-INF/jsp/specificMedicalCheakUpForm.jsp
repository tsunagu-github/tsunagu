...
<head>
    <script type="text/javascript">
            ...
            window.onload = function() {
                document.specificMedicalCheakUpForm.file_button.focus(); 

            };

            function download(){
                var target = document.getElementById('specificMedicalCheakUpForm');
            ...
            function insertButtonClick(){
                ...
                if(fileName.length===0){       // ファイル未選択の時
                    ...
                    //return false;
                } else if(type[type.length-1].toLowerCase()!=='xml' 
                        && type[type.length-1].toLowerCase()!=='zip'){   // xml or zipファイル以外の時
                    document.getElementById("resultArea").textContent="xml形式またはzip形式以外のファイルは読み込めません。";    
                    //return false;
                } else {
                    document.getElementById("resultArea").textContent="";
                    jConfirm("特定健診の登録処理を開始します。","登録処理実施確認",function(r){
                        if(r){
                            var title = "特定健診の登録処理中です。";
                            waitMethod(title);
                            submitForm("insert2");
                        }
                    });
...
<body>
    <form:form modelAttribute="specificMedicalCheakUpForm" name ="specificMedicalCheakUpForm" action="${action}" enctype="multipart/form-data">
        <form:hidden path="command"/>
        <form:hidden path="formCd"/>
        <form:hidden path="formName"/>
        <form:hidden path="param1"/>
        
        <div class="headinglv01"><h1>特定健診登録画面<span>特定健診の登録を行います。</span></h1></div>
        ...
        <div id="uploadStatusInfo" >
            ...
            <div class="scroll" style="height:300px;">
                <table id="resultArea">
                    <tr>
                        <td>
                            <c:forEach var="t" items="${specificMedicalCheakUpForm.insertResulrArea}">
                                <c:out value="${t}"/>
                                <br>
                            </c:forEach>
                        </td>
                ...
                <c:if test="${not empty specificMedicalCheakUpForm.observationEntryDto}">
                <c:if test="${specificMedicalCheakUpForm.observationEntryDto[0].updateFlg==false}">
                <table id="selectArea" class="fmt03 margin-auto">
                    ...
                    <tbody>
                        <c:forEach var="dto" varStatus="status" items="${specificMedicalCheakUpForm.observationEntryDto}">
